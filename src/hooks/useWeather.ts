...
const fetchWeatherDataCity = async (city:string):Promise<CurrentWeather> => {
    try{
        const data = JSON.parse(await fetchWeatherByCity(city));
        console.log(data);
        const unprocessedWeather = JSON.parse(await fetchWeatherByCoordinates(data.coord.lat, data.coord.lon));
...
async function fetchWeatherDataCoordinates(coordinates:coordinates):Promise<CurrentWeather>
{
    try{
        const unprocessedWeather = JSON.parse(await fetchWeatherByCoordinates(coordinates.lat, coordinates.lon));
        const weather: CurrentWeather = {
            date: (new Date(unprocessedWeather.dt * 1000)).toLocaleDateString("en-GB", {
                                                                        weekday: "short",
                                                                        day: "numeric",
